...
dbapi.setUsageLogs = function(serial, user, session) {
  return db.run(r.table('usageLogs').insert({
    session: session,
    user: {
      name: user.name,
      email: user.email
...
dbapi.unsetUsageLogs = function(session) {
  return db.run(r.table('usageLogs').get(session).update({

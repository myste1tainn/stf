...
module.exports.socket = function() {
  console.log('zmqutil.socket called, arguments', arguments)
  var sock = zmq.socket.apply(zmq, arguments);
  ['ZMQ_TCP_KEEPALIVE', 'ZMQ_TCP_KEEPALIVE_IDLE'].forEach(function(opt) {
    if (process.env[opt]) {
      console.log('zmqutil.socket called, opt', opt)
      console.log('zmqutil.socket called, process.env[opt]', process.env[opt])
      try {

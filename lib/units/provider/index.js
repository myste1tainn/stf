...
module.exports = function(options) {
  ...
  client.trackDevices().then(function(tracker) {
    ...
    // ARNON: When ADB client's tracker found a new device connecting to it
    // it notifies the system with DeviceIntroductionMessage and save all of the info
    // into DB
    ...
    tracker.on('add', filterDevice(function(device) {
      ...
      var register = new Promise(function(resolve) {
        ...
        // ARNON: This where the discovered device is being sent
        // through socket and initially write to database
        push.send([
          wireutil.global
      ...
      function spawn() {
        ...
        var slicedPorts = allocatedPorts.slice()
        console.log('### device worker spawn', device)
        console.log('### device worker ports', ports)
        console.log('### device worker allocated ports', allocatedPorts)
        console.log('### device worker sliced ports', slicedPorts)
        var proc = options.fork(device, slicedPorts)

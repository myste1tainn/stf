...
// ARNON: Add capability to have session usage time
var serialsSession = {}
    ...
    .then(function(device) {
      ...
      var messageListener = wirerouter()
        .on(wire.LeaveGroupMessage, function(channel, message) {
          // ARNON: Add capability to have session usage time
          if (!serialsSession[req.user.name]) {
            serialsSession[req.user.name] = {}
          }
          var session = serialsSession[req.user.name][message.serial]
          dbapi.unsetUsageLogs(session)
          delete serialsSession[req.user.name][message.serial]
          delete serialsSession[req.user.name]
    ...
    .then(function(device) {
      ...
      var messageListener = wirerouter()
        .on(wire.ConnectStartedMessage, function(channel, message) {
          ...
          var session = 'usage_session_' + uniqid()
          serialsSession[req.user.name][message.serial] = session
          dbapi.setUsageLogs(message.serial, req.user, session)
